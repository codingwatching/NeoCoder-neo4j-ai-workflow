...
def check_incarnation_registry() -> tuple[Any | None, Any | None]:
    ...
    try:
        ...
        for inc_type, inc_class in registry.incarnations.items():
            logger.info(
                f"  {inc_type}: {inc_class.__name__} - {getattr(inc_class, 'description', 'No description')}"
            )
...
def find_incarnation_modules() -> list[str]:
    """Find all incarnation modules in the codebase."""
    incarnation_modules = []

    # Check each incarnation file
    incarnations_dir = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "incarnations"
    )
...
def examine_incarnation_module(module_name: str) -> list[tuple[str, type]]:
    ...
    try:
        ...
        for name, obj in inspect.getmembers(module):
            if (
                ...
                if hasattr(obj, "_tool_methods") and isinstance(
                    obj._tool_methods, list
                ):
                ...
                for method_name, method in inspect.getmembers(
                    ...
                    if (
                        ...
                        and method_name not in tool_methods
                    ):
...
def test_tool_registration() -> int:
    ...
    try:
        ...
        class MockDriver:
            async def session(self, database: str | None = None) -> "MockSession":
                return MockSession()
        ...
        class MockSession:
            ...
            async def execute_read(
                self, func: object, query: object, params: object = None
            ) -> str:
                return "[]"

            async def execute_write(
                self, func: object, query: object = None, params: object = None
            ) -> None:
                return None
        ...
        for inc_type, _inc_class in inc_registry.incarnations.items():
            ...
            logger.info(
                f"  {inc_type}: Expected {len(tool_methods)} tools, registered {registered_count}, actually added {actual_added}"
            )
        ...
        logger.info(f"{'='*80}")
        logger.info(
            f"{'Incarnation':<25} {'Expected':<10} {'Registered':<12} {'Added':<8}"
        )
        logger.info(f"{'-'*80}")

        for summary in incarnation_summary:
            logger.info(
                f"{summary['name']:<25} {summary['expected']:<10} {summary['registered']:<12} {summary['added']:<8}"
            )
...
def main() -> None:
    ...
    initial_tool_count = (
        len(initial_tool_registry.tools) if initial_tool_registry else 0
    )
