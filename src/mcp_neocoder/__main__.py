...
def main() -> None:
    ...
    logger.info(
        f"NEO4J_USERNAME from environment: {os.environ.get('NEO4J_USERNAME', 'Not set')}"
    )
    logger.info(
        f"NEO4J_PASSWORD from environment: {'Set' if os.environ.get('NEO4J_PASSWORD') else 'Not set'}"
    )
    logger.info(
        f"NEO4J_DATABASE from environment: {os.environ.get('NEO4J_DATABASE', 'Not set')}"
    )
